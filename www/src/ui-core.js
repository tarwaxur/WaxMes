...
async function sendFriendRequest(){
  ...
  function catchErr(label,err){
    ...
    fail(tr('login_failed')+' ('+(err&&err.code?err.code:'?')+'): '+(err&&err.message?err.message:tr('unknown')))
  }
  try {
    // Step 1: Search user
    resetTimer();
    ...
    if(targetUser.id===fbUid){fail(tr('invalid_email'));return}
    ...
    if(!fs.empty){fail(tr('friends')+' '+tr('already'));return}
    ...
    if(pendingCount>=5){fail(tr('too_many_attempts'));return}
    if(alreadySent){fail(tr('no_pending'));return}
    // Step 4: Send request
    var reqId=uid();
    var myAv=null;
...
function showFriendMenu(e,name,friendId){
  ...
  showContextMenu(e.clientX,e.clientY,[
    ...
    {label:tr('delete'),icon:'<svg width="14" height="14" viewBox="0 0 24 24" stroke="currentColor" fill="none" stroke-width="1.5"><path d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 00-3-3.87"/><line x1="1" y1="1" x2="23" y2="23"/></svg>',action:function(){removeFriend(friendId,name)}},
  ])
}
...
var snap=await db.collection(COLLECTIONS.FRIENDS).doc(uid).collection(COLLECTIONS.LIST).get();
    ...
    content.innerHTML='<div style="margin-bottom:14px"><label style="font-size:11px;font-weight:600;color:var(--text2);display:block;margin-bottom:6px">'+tr('add_friend')+'</label><input type="text" id="add-friend-input" placeholder="'+tr('search_by_username')+'" style="width:100%;padding:10px 14px;background:var(--input-bg);border:1px solid var(--border2);border-radius:10px;font-family:inherit;font-size:13px;color:var(--text2);outline:none;margin-bottom:8px"><button class="btn-primary" data-action="send-friend-request" style="padding:8px 16px;font-size:11px;border-radius:8px;width:100%">'+tr('add_friend')+'</button><div id="add-friend-result" style="margin-top:8px;font-size:11px;color:var(--text4)"></div></div>'
  }
}
...
function renderGroupMembers(selectedIds){
  ...
  function addItem(member,convId,color){
    ...
    var av=member.avatar,html;if(av&&av.length>2){html='<img src="'+escJs(sanitizeUrl(av))+'" style="width:100%;height:100%;object-fit:cover" data-err-bg="'+(member.color||color||'var(--grad)')+'" data-err-text="?" data-err-avatar="1">'}else{html='<span>'+esc(av||((member.name||'?').charAt(0).toUpperCase()))+'</span>'}
...
function clearConversation(id){
  ...
  if(conv&&(!conv.lastMsg||conv.lastMsg===tr('chat_cleared')||conv.lastMsg==='Sohbet temizlendi')){
  ...
  body.innerHTML='<svg width="40" height="40" viewBox="0 0 24 24" stroke="#ef4444" fill="none" stroke-width="1.5" style="margin-bottom:12px"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 01-2 2H7a2 2 0 01-2-2V6m3 0V4a2 2 0 012-2h4a2 2 0 012 2v2"/></svg>'+
    ...
    '<p style="color:var(--text4);font-size:12px">'+tr('delete_all')+'</p>';
