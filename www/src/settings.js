...
async function showSettingsCat(cat){
  ...
  if(cat==='profile'){
    ...
    if(!acc){content.innerHTML='<div class="stitle">'+tr('profile')+'</div><p style="color:var(--text4)">'+tr('account_info')+'</p>';return}
    ...
    content.innerHTML='<div class="stitle">'+tr('themes')+'</div><div class="stitle-sub">'+tr('choose_category')+'</div><div class="theme-grid">'+
      ['default','royal','forest','wine','slate','plum','coffee','teal','ember','navy','emerald','cloud','pearl','mist','cream','sage','lilac','coral','sky','linen','frost'].map(function(t){return '<div class="theme-card tp-'+t+(cur===t?' active':'')+'" data-theme="'+t+'" data-action="select-theme"><div class="theme-card-preview"><div class="tcp-dot"></div><div class="tcp-bar"><div></div><div></div><div></div></div></div><div class="theme-card-name">'+tr('theme_'+t)+'</div></div>'}).join('')+
      '</div>'
  }else if(cat==='language'){
    var currentCode = getCurrentLang();
    var langs = getLanguages();
    var html = '<div class="stitle">'+tr('select_language')+'</div>';
    html += '<div style="margin-bottom:18px;padding:16px;border-radius:12px;background:var(--surface);border:1px solid var(--border);cursor:pointer;transition:all .15s" id="use-device-lang">';
    html += '<div style="display:flex;align-items:center;gap:12px"><span style="font-size:20px">🖥</span><div><div style="font-size:13px;font-weight:600;color:var(--text2)">'+tr('use_device_lang')+'</div><div style="font-size:11px;color:var(--text4)">'+detectSystemLanguage()+'</div></div></div></div>';
    html += '<div class="stitle-sub" style="margin-bottom:12px">'+tr('available_languages')+'</div>';
    html += '<div style="display:flex;flex-direction:column;gap:6px" id="lang-list">';
    for (var li = 0; li < langs.length; li++) {
      var l = langs[li];
      var isActive = l.code === currentCode;
      html += '<div data-lang="'+l.code+'" style="display:flex;align-items:center;justify-content:space-between;padding:12px 16px;border-radius:12px;background:'+(isActive?'var(--accent)18':'var(--surface)')+';border:1px solid '+(isActive?'var(--accent)40':'var(--border)')+';cursor:pointer;transition:all .15s">';
      html += '<div style="display:flex;align-items:center;gap:12px"><span style="font-size:24px">'+l.flag+'</span><div><div style="font-size:13px;font-weight:600;color:var(--text2)">'+l.name+'</div><div style="font-size:10px;color:var(--text4);text-transform:uppercase">'+l.code+'</div></div></div>';
      if (isActive) html += '<svg width="20" height="20" viewBox="0 0 24 24" stroke="var(--accent)" fill="none" stroke-width="2.5"><polyline points="20 6 9 17 4 12"/></svg>';
      html += '</div>';
    }
    html += '</div>';
    content.innerHTML = html;
    setTimeout(function(){
      var deviceBtn = $('use-device-lang');
      if(deviceBtn) deviceBtn.onclick = function(){ setLanguage(detectSystemLanguage()) };
      var langItems = document.querySelectorAll('#lang-list [data-lang]');
      langItems.forEach(function(el){ el.onclick = function(){ setLanguage(el.dataset.lang) } });
    }, 0);
  }else if(cat==='privacy'){
    ...
    content.innerHTML='<div class="stitle">'+tr('privacy_security')+'</div>'+
      ...
      '</div>'+
      '<div class="field-group"><label>'+tr('email_label')+' <span style="font-size:9px;color:var(--text4);font-weight:400">'+tr('email_disabled')+'</span></label><input type="email" id="set-email" value="'+(function(){var a=getAccounts();for(var i=0;i<a.length;i++){if(a[i].id===store.activeAccountId)return esc(a[i].email)}return''})()+'"></div>'+
    ...
    var micOptions='<option value="">'+tr('settings')+'</option>',spkOptions='<option value="">'+tr('settings')+'</option>';
    ...
    content.innerHTML='<div class="stitle">'+tr('media')+'</div>'+
      '<div class="stitle-sub" style="margin-bottom:18px">'+tr('media')+'</div>'+
      '<div style="display:flex;flex-direction:column;gap:14px">'+
        ...
        '<div style="padding:18px;border-radius:14px;background:var(--surface);border:1px solid var(--border);transition:all .2s">'+
          ...
          '<div id="screen-share-preview" style="width:100%;aspect-ratio:16/9;border-radius:10px;background:var(--bg3);display:none;align-items:center;justify-content:center;margin-top:10px;overflow:hidden;border:1px solid var(--border2)">'+
            '<span style="font-size:11px;color:var(--text4);opacity:.6">'+tr('screen_share')+'</span>'+
          '</div>'+
        '</div>'+
        '<div style="padding:18px;border-radius:14px;background:var(--surface);border:1px solid var(--border);transition:all .2s">'+
          '<div style="font-size:22px;margin-bottom:8px">🔇</div>'+
          ...
          '<div style="display:flex;align-items:center;gap:10px;margin-bottom:10px">'+
            '<label class="toggle"><input type="checkbox" id="noise-toggle" '+(ls(STORAGE_KEYS.NOISE_SUPPRESSION)?'checked':'')+' data-action="toggle-noise"><span class="toggle-track"></span><span class="toggle-label" style="font-size:11px;color:var(--text4)">'+tr('noise_suppression')+'</span></label>'+
          '</div>'+
    ...
    content.innerHTML='<div class="stitle">'+tr('about')+'</div>'+
      ...
      '<b style="color:var(--text3)">'+tr('technologies')+'</b><br>• Electron • Vanilla JS • CSS3<br><br>'+
...
async function checkUpdate(){
  ...
  if(window.electronAPI && electronAPI.checkForUpdates){
    ...
    showUpdateBar(tr('checking'), 'info');
    try {
      ...
      if(result && result.error){
        ...
        btn.disabled = false;
        showUpdateBar(tr('update_error')+result.error, 'error');
        return;
      }
      if(result && result.updateAvailable){
        ...
        btn.dataset.found = '1';
        btn.disabled = false;
        showUpdateBar(tr('update_available')+' v'+result.version+' ('+tr('current_ver')+' v'+result.currentVersion+'). '+tr('click_to_download')+'.', 'info');
...
showUpdateBar(tr('download_error')+': '+(resp&&resp.error?resp.error:tr('unknown')), 'error');
      ...
      btn.textContent = tr('check_updates');
      btn.disabled = false;
      showUpdateBar('Check failed: '+(err&&err.message?err.message:err), 'error');
    }
  } else {
    store._updateCheckLock = false;
...
function clearLocalData(){
  ...
  body.innerHTML='<svg width="40" height="40" viewBox="0 0 24 24" stroke="var(--text4)" fill="none" stroke-width="1.5" style="margin-bottom:12px"><path d="M21 4H8l-1 2H3v2h18V4z"/><line x1="10" y1="12" x2="10" y2="18"/><line x1="14" y1="12" x2="14" y2="18"/><path d="M5 6v13a2 2 0 002 2h10a2 2 0 002-2V6"/></svg>'+
    ...
    '</label>';
  $('delete-confirm-btn').textContent=tr('delete_local_data');
