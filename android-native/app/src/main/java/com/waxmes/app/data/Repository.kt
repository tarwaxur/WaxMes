...
class Repository {
    ...
    suspend fun register(email: String, password: String, name: String): Boolean = try {
        ...
        uid.let { db.collection("users").document(it).set(mapOf("displayName" to name, "email" to email, "avatar" to name.first().uppercase(), "online" to true)).await() }
        true
    ...
    suspend fun getConversations(): List<Conversation> {
        ...
        val snap = db.collection("conversations").whereArrayContains("memberIds", uid).get(Source.SERVER).await()
        return snap.documents.mapNotNull { doc ->
            ...
            Conversation(
                id = doc.id, name = d["name"] as? String ?: otherId,
                lastMsg = (d["lastMsg"] as? String) ?: "",
                lastActivity = ((d["lastActivity"] as? com.google.firebase.Timestamp)?.toMillis() ?: (d["lastActivity"] as? Long ?: 0L)),
    ...
    fun listenMessages(convId: String, onChange: (List<Message>) -> Unit) = db.collection("conversations").document(convId).collection("messages")
        .orderBy("createdAt", Query.Direction.ASCENDING).addSnapshotListener { snap, _ ->
            ...
            onChange(snap.documents.map { doc ->
                val d = doc.data ?: return@mapNotNull doc
    ...
    suspend fun sendMessage(convId: String, text: String) {
        val msg = mapOf("text" to text, "time" to java.text.SimpleDateFormat("HH:mm", java.util.Locale.getDefault()).format(java.util.Date()),
            "senderId" to uid, "createdAt" to com.google.firebase.firestore.FieldValue.serverTimestamp())
        db.collection("conversations").document(convId).collection("messages").add(msg).await()
        db.collection("conversations").document(convId).set(mapOf("lastMsg" to text, "lastActivity" to com.google.firebase.firestore.FieldValue.serverTimestamp()), com.google.firebase.firestore.SetOptions.merge()).await()
    }
}